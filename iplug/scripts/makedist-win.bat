...
if %DEMO_ARG% == "demo" (
  echo Making Curvessor Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making Curvessor Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild Curvessor-app.vcxproj /p:configuration=release /p:platform=x64

echo Building x64 binaries...
REM add projects with /t to build VST2 and AAX
msbuild Curvessor.sln /t:Curvessor-app;Curvessor-vst3;Curvessor-clap /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
msbuild Curvessor.sln /t:Curvessor-app;Curvessor-vst3;Curvessor-clap /p:configuration=release /p:platform=ARM64EC /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win-arm64ec.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Curvessor.aaxplugin\Contents\Win32\Curvessor.aaxplugin --out .\build-win\aax\bin\Curvessor.aaxplugin\Contents\Win32\Curvessor.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Curvessor.aaxplugin\Contents\x64\Curvessor.aaxplugin --out .\build-win\aax\bin\Curvessor.aaxplugin\Contents\x64\Curvessor.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\Curvessor.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\Curvessor.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Curvessor Installer" ".\installer\Curvessor Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\Curvessor Installer.exe" ".\installer\Curvessor Demo Installer.exe"
  REM -del ".\installer\Curvessor Installer.exe"
  REM -)
